...
public class VisitHistory extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (session.getAttribute("user") == null || !((String) session.getAttribute("user")).equals(request.getParameter("user_id"))) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (input.has("user_id") && input.has("visited")) {
				...
				// allow access only if session exists
				HttpSession session = request.getSession();
				if (session.getAttribute("user") == null || !((String) session.getAttribute("user")).equals(userId)) {
					response.setStatus(403);
					return;
				}
